...
def _circulation_search(sender, data):
    def wrapped():
        ...
        item_ids += [str(x.id) for x in m.CirculationItem.search(search)]
        user_ids += [str(x.id) for x in m.CirculationUser.search(search)]
        record_ids += [str(x.id) for x in m.CirculationRecord.search(search)]
...
def _circulation_state(sender, data):
    def wrapped():
        ...
        from invenio.modules.circulation.receivers.utils import _try_action
